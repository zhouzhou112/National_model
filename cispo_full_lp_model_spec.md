...
> 数值坐标候选（`annual_energy_coordinate_8192_v1`，默认关闭）：只把年度账户与省内年度流量变量的内部单位改成 `8192 GWh`，即 `E_physical = 8192 e_internal`，并把连接逐小时量或物理GWh量的相应年度账户行乘以 `1/8192`。负荷中心VRE、波浪能、径流式和水库年度发电变量及其资源可用量行仍保持物理GWh坐标，避免把极小CF/径流汇总系数进一步缩小。目标中的省内流量破简并成本同步乘以 `8192`；公开输出和QC全部乘回物理GWh。风光小时CF、径流小时CF、水库入流、逐小时发电/负荷、输电物理成本与惩罚系数的经济含义均不修改或截断。原始solver中被缩放账户/流量的primal、相关行dual和对应reduced-cost处于内部坐标，分别按 `D`、`R^T`、`D^{-T}`恢复后才可解释；资源发电变量与资源行不作该变换。该配置是待A/B验证的代数等价候选，不因局部range改善自动获得生产接受。
