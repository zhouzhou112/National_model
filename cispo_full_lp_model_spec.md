...
> 方向性 QC 分为两个不可混淆的范围。`SCIENTIFIC_PRODUCTION` 全年结果继续要求所有 AC edge-hour 在 `1e-6 GW` 容差以上严格零对冲，DC 反向流也严格为零。`TEST_ONLY_TRUNCATED_HORIZON` 工程门禁允许把极小系统影响的 AC 对冲流降为显式 warning，但必须同时满足配置中的七重预算：发生 edge-hours、单小时较小方向功率、较小方向/线路容量比例、累计较小方向电量、由对冲流额外制造的输电损耗、其占毛输电量比例及其占系统负荷比例。任一预算超限仍为 hard fail；warning 的原始流量、额外损耗、限额和适用范围全部写入 `solution_qc.json`，不能重标为严格单向，也不能用于年度科学结论。
...
自 2026-08-01 起，超过 744 h 的默认求解合同改为 **Barrier-first**，但不放松科学验收：第一阶段必须使用 `Method=2`、`Crossover=0`、`SolutionTarget=1`，只有最终 `Status=OPTIMAL`、严格 primal/dual quality contract、完整 `solution_qc=PASS`、全部 hard checks、当前 input manifest 与有效 result manifest 同时闭合，才接受内点解。通过后，runner 正常导出容量、逐小时运行、成本、碳/CCS 和 `BarPi` 影子价格，并自动保存按原 LP 顺序排列的 `BarX`/`BarPi` 检查点；744 h 可用 `--export-barrier-checkpoint` 显式启用。检查点不含 presolve 消元或 Barrier factorization，约 8760 h 原始向量本体的 float64 规模约为变量数与约束数之和乘以 8 bytes，导入时仍可能额外物化 Python Gurobi 对象列表。

第二阶段 Crossover 是独立、可选且不可覆盖第一阶段结果的派生任务。它必须重建完全相同的 LP，逐层核对 baseline/analysis case、implementation bundle、data roots、input manifest、规划年、窗口、Gurobi `Fingerprint`、变量/约束/非零元数及检查点 SHA256，再把全量 `BarX/BarPi` 分别作为 `PStart/DStart` 注入，使用 `LPWarmStart=2` 将 start 映射到 presolved model 后执行 `Crossover=1/CrossoverBasis=1`。该阶段用于生成 basis、reduced cost、basis sensitivity、MGA warm start 及更适合跨年容量锚定的极点解；其失败不得撤销第一阶段已经接受的科学结果。为避免内点最优解在高度退化容量空间中的弥散分配被无审查地传入下一规划年，非基第一阶段不自动写出 `planning_state`；必须由独立 Crossover 或另行批准的 anchor 决策闭合。旧式同进程 Crossover 在超过 744 h 时默认被 runner 拒绝，只有显式 `--allow-inline-crossover` 才可作为诊断运行；若 Gurobi 13 已记录 `BarStatus=OPTIMAL` 后整体超时，runner 尝试保存 `RECOVERY_ONLY_UNACCEPTED_SOLVER_RESULT` 原始检查点，但它没有完整 QC，不能被重标为科学接受结果。
