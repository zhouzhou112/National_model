...
# Concurrent matched 744 h factor screens.  These stop after five Barrier
# iterations and are diagnostic only: no solution or scientific acceptance is
# expected.  The pair is isolated from the clean production checkout.
SERVER_ROOT=${CISPO_SERVER_ROOT:-/home/zz2/National_model_server}
REPO_ROOT=$SERVER_ROOT/repo
TOOL_ROOT=$SERVER_ROOT/campaign_tools/cf_744_pair_20260901_v1
...
BASELINE_TAG=2030_base_744h_cf1e6_factor5_concurrent_20260901_v1
CANDIDATE_TAG=2030_base_744h_cf1e4_factor5_concurrent_20260901_v1
...
for profile in \
  "$PROFILE_ROOT/large_lp_744_v3_baseline_factor5_v1.json" \
  "$PROFILE_ROOT/large_lp_744_cf1e4_factor5_v1.json"; do
...
launch_case() {
  ...
  local pid=$!
  printf '%s\n' "$pid" >"$control/run.pid"
  "$PYTHON" scripts/monitor_case_resources.py \
...
launch_case \
  "$BASELINE_TAG" "$PROFILE_ROOT/large_lp_744_v3_baseline_factor5_v1.json" \
  "$BASELINE_CPUS"
baseline_pid=$LAUNCHED_PID
launch_case \
  "$CANDIDATE_TAG" "$PROFILE_ROOT/large_lp_744_cf1e4_factor5_v1.json" \
  "$CANDIDATE_CPUS"
