...
- Git scope: 新增 `config/scenarios/wave_energy_medium_v1_flexible_load_comfort_v3_v2g_5pct.json` 并登记 catalog；仅扩展独立情景和契约测试，未改动 Base JSON、LP 目标函数、约束、空间/时间尺度或既有模块接口。情景同时使用 medium 波浪能、`comfort_envelope_v3` 冷热、V1G 和日内因果 5% V2G；参数状态仍为敏感性，不能提升为 Base 或论文基准。
