...
- Handoff version: `v0.9.3`
- Snapshot date: `2026-07-22`
- Local repository: `D:\codeenv\pycharmproject\National_RL\National_model`
- Git branch: `codex/cispo-2030-full-lp`
- Local implementation commit `c91828a` (`feat: add diagnostic sensitivity suite runner`) is based on accepted handoff commit `0d44616`. It adds a diagnostic-only, checksummed sensitivity-suite runner around the existing four-year sequence without changing any scientific parameters, objectives, constraints, time/space scales or state-transfer rules. Base remains flexibility-disabled, `flexible_load_v1` keeps V2G disabled and `flexible_load_v2g_v1` remains a separate optional scenario. The fixed-server checkout was independently verified clean and idle at model commit `6ed943a`; the new suite runner has not been deployed there and the accepted 168h output identity was not changed. The immediately preceding fixed-server Base gate ran from checkout `b6ca42d`, whose model implementation baseline is `0c1eaf2`.
...
The fixed-server Base 168h and `flexible_load_v1` 24h/168h four-year gates are accepted, and the local diagnostic sensitivity-suite interface is implemented and validated. Do not run 744h or 8760h on the fixed server, and do not launch a paid cloud 8760h case without explicit cost authorization. The next model-development milestone is to define evidence-backed low/base/high envelopes for building thermal/comfort flexibility and province-hour EV availability, driving energy, accessible battery and departure SOC before adding new runnable sensitivity cases. `flexible_load_v2g_v1` remains an independent optional scenario. Point-level, intraprovincial and national complementarity plus cost-relaxed complementarity remain planned-not-runnable until their spatial aggregation, cost decomposition and objective/constraint definitions are frozen.
