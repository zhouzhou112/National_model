...
- Solvability optimization milestone (2026-07-26, uncommitted worktree based on `796a6fc`): the 474 independent non-cascade reservoirs now eliminate their explicit hourly nonnegative spill columns through the exact projection `V_t <= V_{t-1} + (Q_t - q_t) * 3600`; all 146 core-cascade reservoirs retain explicit spill because it enters downstream inflow. Independent spill is reconstructed from water-balance slack for unchanged output arrays and a new nonnegativity hard QC. This removes `474 * 8760 = 4,152,240` columns, reducing the Base full-year estimate from 40,912,327 to 36,760,087 variables (10.15%) without changing constraints. A same-scenario 24h `flexible_load_comfort_v3` comparison reproduces the prior objective within `1.4e-9 million CNY`, passes every QC, has `4.68e-6 m3` maximum water-balance residual and only `-2.12e-9 m3/s` reconstructed-spill numerical noise. Regression passes 68/68. A numerics-only `--solver-config` contract, versioned Barrier/dual-simplex profiles, flushed `solver_telemetry.jsonl` and SIGTERM/SIGINT-to-`Model.terminate()` handling are locally validated; solver-profile paths/hashes are recorded and cannot modify scientific settings.
...
- Handoff version: `v0.9.26`
- Snapshot date: `2026-07-26`
- Local repository: `D:\codeenv\pycharmproject\National_RL\National_model`
- Git branch: `codex/cispo-2030-full-lp`
- Local state-flex implementation commit is `271c6dc` (`feat: add state-based flexible load scenario`); the documentation follow-up may place local HEAD one commit later. Fixed-server model HEAD remains last verified as `cf39e0a` (`fix: align fuel-cost smoke contract`), containing the `8e76753` city-network/reliability implementation. The fully rebuilt `city_337` package is the production configuration; 278 Natural Earth remains separate. The fixed server uses the additive data root `/data/zz2/National_model/data/model_ready_20260723_v0722_city337`. No fixed-server or cloud checkout was changed for `271c6dc`.
...
The fixed-server Base 168h and `flexible_load_v1` 24h/168h four-year gates are accepted; the local sensitivity-suite interface, CISPO-equivalent BECCS mass balance and reviewed `5%` capacity-margin / `3.5 s` inertia update are validated locally but the latest reliability update is not deployed. Do not run 744h or 8760h on the fixed server, and do not launch a paid cloud 8760h case without explicit cost authorization. Before treating the four milestone solves as a 2025-2060 total transformation-cost optimum, define and implement an explicit discounted pathway-cost formulation or accounting ledger; the current objective remains a target-year annual system cost. Nuclear lifetime 40/60 years, future CapEx/fuel trajectories and separate line/substation annualization remain open scenario/accounting work. A nonzero BECCS lifecycle factor requires an independently sourced adapted scenario.
...
- Git state: local branch `codex/cispo-2030-full-lp`, dirty worktree based on `796a6fc`; final commit and server deployment are the next controlled milestone. No `supplementary_materials/**` file was modified or staged for this implementation.
...
- Exact next action: commit/push only the reviewed non-supplementary implementation, fast-forward the idle fixed server, pass server regression plus 24h/168h gates, then run one 744h profile at a time under the 48 GiB solver soft limit and compare build, presolve, ordering, factorization, iterations, crossover, memory, objective and QC.
