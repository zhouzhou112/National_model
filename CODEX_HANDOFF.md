...
- Full-year cloud state (live read-only audit at 2026-07-24 21:17 CST): structural job `4003088` completed successfully (`COMPLETED`, exit `0:0`) in 40m20s on `m4cm1904`. Its build report records 40,912,327 variables, 68,189,325 constraints, 515,040,080 nonzeros and 53.633 GiB peak process-tree RSS. The dependent real single-year job `4003172` then started automatically on `m4cg1702` with 128 CPU/700G and a 640G Gurobi soft-memory limit. After about 53 minutes total runtime, its 40m32s model build completed with identical statistics and 53.686 GiB build peak; Gurobi entered `Optimize`, but had not yet logged presolve completion or barrier iterations. Slurm `sstat` reported 96.402 GiB current batch MaxRSS. No `solve_report.json` or `solution_qc.json` existed yet. Do not start a successor year or interpret this active run as accepted.
