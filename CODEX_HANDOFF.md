...
- Solvability A/B milestone (2026-07-26, comparison commit `9e82cc5`): the exact independent-reservoir spill projection from `6d84209` is not accepted as the default merely because it removes 4,152,240 raw full-year columns. On the same local 24h `flexible_load_comfort_v3` case it increased the presolved matrix from 130,555/262,805/1,271,445 to 133,206/257,955/1,280,047 rows/columns/nonzeros, Factor Ops from `5.886e8` to `6.452e8`, and solver time from 34.36 s to 45.17 s. The fixed-server projected Base 168h run is nevertheless `OPTIMAL + solution_qc=PASS` in 641.996 s, so a matched explicit-spill 168h rerun is required before final selection. Current local/default code restores explicit spill and its equality balance; regression passes 67/67 and the repeated 24h objective is exactly `1,982,474.939703926 million CNY`. `scripts/compare_solver_runs.py` now extracts build, presolve, factorization, telemetry, solve and QC metrics. No 744h run should start until this matched A/B closes.
...
- Handoff version: `v0.9.27`
- Snapshot date: `2026-07-26`
- Local repository: `D:\codeenv\pycharmproject\National_RL\National_model`
- Git branch: `codex/cispo-2030-full-lp`
- Local implementation HEAD is `9e82cc5`; `44f7fef` adds the large-LP solver profiles and `9e82cc5` restores explicit reservoir spill as the provisional default while adding automatic solver-run comparison. Fixed-server HEAD is verified as `ed8e91a` and its projected-spill 168h output is complete; the checkout must remain unchanged until the explicit-spill comparison commit is pushed and the host is idle. The fully rebuilt `city_337` package is the production configuration; 278 Natural Earth remains separate. The fixed server uses the additive data root `/data/zz2/National_model/data/model_ready_20260723_v0722_city337`.
