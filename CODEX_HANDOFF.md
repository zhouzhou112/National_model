...
- Local implementation commit is `6d84209` (`feat: optimize full-year solvability and optional modules`), containing the validated comfort V3, optional wave, exact independent-reservoir spill projection, solver profiles and telemetry. Fixed-server model HEAD remains last verified as `cf39e0a` (`fix: align fuel-cost smoke contract`) until the next controlled deployment. The fully rebuilt `city_337` package is the production configuration; 278 Natural Earth remains separate. The fixed server uses the additive data root `/data/zz2/National_model/data/model_ready_20260723_v0722_city337`.
