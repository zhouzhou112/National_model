...
- Handoff version: `v0.9.4`
- Snapshot date: `2026-07-22`
- Local repository: `D:\codeenv\pycharmproject\National_RL\National_model`
- Git branch: `codex/cispo-2030-full-lp`
- Local implementation commit `c62b769` (`fix: close beccs carbon mass balance`) is based on sensitivity-suite handoff `231c3d9`. It preserves the CISPO BECCS net factors, 90% capture assumption, capture/transport/storage costs and feasible set while explicitly separating gross, captured, stored and uncaptured biogenic CO2, zero-baseline lifecycle emissions and net removal with hard closure QC. The fixed-server checkout remains unchanged at accepted model commit `6ed943a`; neither `c91828a` nor `c62b769` has been deployed there, and no accepted server output was modified.
...
The fixed-server Base 168h and `flexible_load_v1` 24h/168h four-year gates are accepted; the local sensitivity-suite interface and CISPO-equivalent BECCS mass balance are implemented and validated but not deployed. Do not run 744h or 8760h on the fixed server, and do not launch a paid cloud 8760h case without explicit cost authorization. Before another paper baseline, define explicit `cispo_replication` versus `adapted_main` scenarios for nuclear lifetime 40/60 years, capacity margin 5/15% and inertia reference/tolerance rather than silently changing Base. Future CapEx and fuel-price low/base/high trajectories and separate line/substation annualization remain open. A nonzero BECCS lifecycle factor requires an independently sourced adapted scenario.
