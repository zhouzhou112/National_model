...
- Fixed-server 24h `city_337` `flexible_load_v1` sequence `/data/zz2/National_model/outputs/planning_sequence_24h_v0722_city337_flexible_load_v1` and immediate `--resume` pass. All four years are `OPTIMAL + solution_qc=PASS`; runtimes are 40.84/44.23/43.39/41.95 s, wall time 9:35, peak RSS 929,096 KiB and zero swaps. Daily heating/cooling/EV V1G residuals are at most `1.14e-13 GWh`, simultaneous up/down counts are zero, V2G is disabled and every 59-entry manifest is complete. PID `3687503` now runs the corresponding new 168h V1 chain; keep checkout `cf39e0a` and data immutable while it runs.
