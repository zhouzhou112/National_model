...
## 2026-09-02 10:20 数值稳定性工程线已隔离建立，未部署

- 版本关系已核实：服务器clean6065bfb是本地主分支/裸远端65c8b21祖先，本地领先两条运行记录，
  没有分叉。新工程分支从6065bfb建立，避免改变活动Stage B身份。
- 统一工程记录为`NUMERICAL_STABILITY_ENGINEERING_LINE.md`；整合的候选均默认关闭、禁止叠加。
- 744h CF1e-4候选当前尺度完整Stage A较慢，因此不自动晋级；该结果不外推为2160h/8760h必慢。
  后续只允许在Case1/Case2结束和资源门禁满足后做隔离2160h结构/有限Barrier复测。
- 本工程线尚未部署固定服务器、未修改活动进程/数据/参数/输出。服务器实时状态仍以生产
  `CODEX_HANDOFF.md`及run_control为准，不从本分支旧状态段启动任务。
