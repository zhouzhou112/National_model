...
## 2026-07-29 统一候选已部署；唯一 2030 V4 V1G 744 h cold gate 已进入 Crossover

- 当前固定服务器 checkout 为干净 `codex/cispo-2030-full-lp` / `7c56622c266e673037bd6afaa70c85aa57e6cb13`；本次审计开始时，本地、固定服务器 bare `origin` 与 GitHub 均同步于模型实现基线 `7aac739e03646edfed14bbf48ac77869ba66cbef`。本次后续只增加交接文档；服务器与该实现基线的模型代码一致，进程退出前不切换服务器 checkout。最终外置输入根为 `/data/zz2/National_model/data/model_ready_20260729_unified_7c56622_v4`，标准归档 SHA256 `f3e6cc0f810d0f4e1ccf8fd10907fb25b8859546be397f21035cd072cdb9d261`。`model_input_files.json` v10 包含 42 张运行表和 12 个 sidecar，显式覆盖 wave、V3、V4 和其来源 manifest。
...
- 唯一活动根 `/data/zz2/National_model/outputs/2030_744h_v0729_unified_v4_v1g_cold_v1` 使用 `--planning-year 2030 --horizon one_month`、`flexible_load_comfort_v4_v1g`、`barrier_16_auto_order_v2`，没有 basis。控制 PID/日志在 `/data/zz2/National_model/run_control/2030_744h_v0729_unified_v4_v1g_cold_v1`。这是单独的 2030 cold gate，不会自动求解 2040/2050/2060；`scripts/run_cispo_planning_sequence.py` 虽存在但未被当前命令调用。raw 为 `5,238,323/3,942,756/42,266,264`，presolved 为 `3,234,057/2,812,319/31,818,664`，presolve/ordering `177.00/108.70 s`，`AA' NZ=6.236e7`、`Factor NZ=8.124e8`、`Factor Ops=5.567e12`。
- 2026-07-29 15:32:55+08:00 live snapshot：wrapper/Python PID `1004972/1004975` 仍存在。Barrier 已在 `313` 次迭代、`9396.13 s` 完成，interior objective 约 `2,330,214.46 million CNY`，随后日志明确进入生产 `Crossover=1`。crossover basis 加入后最近进度为 `922,426 DPushes remaining`、`DInf=1.9217246e-6`；telemetry phase 为 `simplex`、runtime `9604.40 s`、current/max solver memory `14.06/23.13 GiB`。主机约 `101 GiB` available、swap `781 MiB/2.0 GiB`；连续 `vmstat si/so=0`，memory PSI avg10/60/300 均为 0。`solve_report.json`、`solution_qc.json`、`result_manifest.json` 尚不存在，当前只能分类为“进入 Crossover”，不是终态成功。
- 同时刻 ParaCloud `squeue -u a8s001819` 为空；历史 `4003088=COMPLETED build-only`、`4003172=OUT_OF_MEMORY`、`4004585=TIMEOUT`。另有一个旧 release-audit SSH/bash/grep 管道，无 Python/Gurobi 子进程且不是第二求解，本次未终止。当前运行锁：不得启动第二个 CISPO/Gurobi、固定服务器 8760 h、付费云、basis gate 或 `Crossover=3`。本条 Crossover 进度会迅速过期；后续必须先读取 live telemetry、进程、RSS/swap/PSI 与终态文件，不能把“进入 Crossover”重标为成功。
