...
- Barrier-16 automatic order is the current leader: `OPTIMAL + solution_qc=PASS` in 637.344 s, unchanged reference factor structure, 3.538 GB telemetry solver memory and 3.459 GiB process-tree peak. The first CPU-PDHG launch was rejected pre-build by a missing `pdhg_gpu` whitelist key; this code defect and regression coverage are fixed locally, and the failed root contains no model result.
