...
## 2026-07-29 统一候选已部署；唯一 2030 V4 V1G 744 h cold gate 正在运行
...
- 2026-07-29 15:18:36+08:00 最终窗口 live snapshot：wrapper/Python PID `1004972/1004975`，elapsed 约 `2:31 h`；Barrier iteration `280`、runtime `8746.80 s`、telemetry primal/dual infeasibility `0.014279/6.29e-7`、complementarity `1.31e-9`，solver current/max memory `13.56/23.13 GiB`。最近 Gurobi 表的 primal residual 仍约 `4.6e-3--4.9e-3`，因此尚未数值闭合。主机约 `95 GiB` available、swap `781 MiB/2.0 GiB`；`vmstat` 无新增 swap-in/out，memory PSI avg10/60/300 均为 0。`solve_report.json`、`solution_qc.json`、`result_manifest.json` 尚不存在。
- 同时刻 ParaCloud `squeue -u a8s001819` 为空；历史 `4003088=COMPLETED build-only`、`4003172=OUT_OF_MEMORY`、`4004585=TIMEOUT`。当前运行锁：不得启动第二个 CISPO/Gurobi、固定服务器 8760 h、付费云、basis gate 或 `Crossover=3`。本条 Barrier iteration 会迅速过期；下一窗口必须先读取 live telemetry、进程、RSS/swap/PSI 与终态文件，不能把“正在运行”重标为成功。
